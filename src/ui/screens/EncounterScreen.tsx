...
export const EncounterScreen = () => {
  const { playerMode, selectedNode, selectedEncounter, resolveSelectedEncounter, resolveSelectedNodeDirectly } = useGameState()
  const [resultSummary, setResultSummary] = useState<string>('')

  if (!selectedEncounter && !selectedNode) {
    return (
      ...
      {selectedEncounter ? (
        <>
          <div className="rounded border border-slate-300 p-3 text-sm">
            ...
            <p className={colorByDanger[selectedEncounter.danger.label]}>Danger: {selectedEncounter.danger.label}</p>
        ...
        <div className="rounded border border-slate-300 p-3 text-sm">
          <p className="font-medium">{selectedNode?.reward ?? selectedNode?.id}</p>
          <p>Node type: {selectedNode?.type}</p>
          <p>Direct interaction available in Dev Mode.</p>
        </div>
      )}
          ...
          <>
            <button
              ...
              onClick={() => {
                const outcome = resolveSelectedEncounter()
                if (!outcome) return
                setResultSummary(`Force Engage complete · ${outcome.result.winner}`)
              }}
            >
              Force Engage
            </button>
            <button
              ...
              type="button"
              onClick={() => {
                const outcome = resolveSelectedNodeDirectly()
                if (!outcome) return
                setResultSummary(`Force Resolve complete · ${outcome.result.winner} · Loot ${outcome.loot.length}`)
              }}
            >
              Force Resolve
            </button>
